...
class VQA_Classifier(VQA_DataLoader):
    ...
    def create_vit_classifier(self, num_projection_layers, projection_dims, dropout_rate):

        # Compute the mean and the variance of the training data for normalization.
        #data_augmentation.layers[0].adapt(self.train_ds)
        
        img_input = layers.Input(shape=self.IMAGE_SHAPE, name="image_input")
        #img_input = layers.Input(shape=self.input_shape, name="image_input")
        # Augment data.
        #augmented = data_augmentation(img_input)
        # Create patches.
        patches = Patches(self.patch_size)(img_input)
        # Encode patches.
        ...
        for _ in range(self.transformer_layers):
            ...
            attention_output = layers.MultiHeadAttention(
                num_heads=self.num_heads, key_dim=projection_dims, dropout=0.2
            )(x1, x1)
            # Skip connection 1.
            ...
            # MLP.
            x3 = mlp(x3, hidden_units=self.transformer_units, dropout_rate=0.2)
            # Skip connection 2.
        ...
        outputs = self.project_embeddings(representation, num_projection_layers, projection_dims, dropout_rate)
    ...
    def build_classifier_model(self):
        ...
        img_input, vision_net = self.create_vit_classifier (num_projection_layers=1, projection_dims=128, dropout_rate=0.1)
