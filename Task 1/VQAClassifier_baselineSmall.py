...
class VQA_Classifier(VQA_DataLoader):
    ...
    def create_vit_classifier(self, num_projection_layers, projection_dims, dropout_rate, vanilla=False):
    ...
    def build_classifier_model(self):
        ...
        img_input, vision_net = self.create_vit_classifier (num_projection_layers=1, projection_dims=128, dropout_rate=0.8)
        text_input, text_net = self.create_text_encoder(num_projection_layers=1, projection_dims=128, dropout_rate=0.8)
        net = tf.keras.layers.Concatenate(axis=1)([vision_net, text_net])
        net = tf.keras.layers.Dropout(0.8)(net)
        net = tf.keras.layers.Dense(num_classes, activation='softmax', name=classifier_model_name)(net)
